...
#import "WYAJHPieCenterView.h"
#import "WYAJHPieModel.h"

#define COLOR_ARRAY @[\
[UIColor colorWithRed:255/255.0 green:59/255.0 blue:48/255.0 alpha:1],\
[UIColor colorWithRed:89/255.0 green:90/255.0 blue:211/255.0 alpha:1],\
...
#define CHART_MARGIN 65

@interface WYAJHPieChart ()
...
-(void)drawRect:(CGRect)rect{
    ...
    CGFloat radius = min * 0.5 - CHART_MARGIN*Size_ratio;
    CGFloat start = 1.5*M_PI;
    CGFloat angle = 0;
    CGFloat end = start;
    ...
    if (self.dataArray.count == 0) {
        ...
        for (int i = 0; i < self.dataArray.count; i++) {
            WYAJHPieModel *model = self.dataArray[i];
            if (model.rate) {
    ...
    // 在中心添加label
    WYAJHPieCenterView *centerView = [[WYAJHPieCenterView alloc] init];
    centerView.frame = CGRectMake(0, 0, 120*Size_ratio, 120*Size_ratio);
    centerView.layer.cornerRadius = 60*Size_ratio;
...
- (void)addLineWithColor:(UIColor *)color X:(CGFloat)x Y:(CGFloat)y name:(NSString *)name number:(NSString *)number radianCenter:(CGFloat)radianCenter {
    ...
    //指引线上面的数字
    [name drawInRect:CGRectMake(numberX, numberY, numberWidth, numberHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10.0*Size_ratio], NSForegroundColorAttributeName:strColor,NSParagraphStyleAttributeName:paragraph}];
    
    // 指引线下面的title
    [number drawInRect:CGRectMake(titleX, titleY, titleWidth, titleHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10.0*Size_ratio],NSForegroundColorAttributeName:strColor,NSParagraphStyleAttributeName:paragraph}];
}
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        // 模型数据（绘制数据时要用）
        WYAJHPieModel *model = self.modelArray[i];
        ...
        CGFloat margin = fabs(width - breakPointX) + 20*Size_ratio;
        ...
        CGFloat numberWidth = 80.f*Size_ratio;
        CGFloat numberHeight = 15.f*Size_ratio;
